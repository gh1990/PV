...
End Sub
'================================
' Verifica daca o foaie este critica (nu trebuie stearsa)
Private Function IsCriticalSheet(ByVal sheetName As String) As Boolean
    Dim criticalSheets As Variant
    Dim i As Long
    
    criticalSheets = Array("Obiect", "Norma", "Materiale", "Utilaj", "Transport", "Liste")
    
    For i = LBound(criticalSheets) To UBound(criticalSheets)
        If StrComp(sheetName, criticalSheets(i), vbTextCompare) = 0 Then
            IsCriticalSheet = True
            Exit Function
        End If
    Next i
    
    IsCriticalSheet = False
End Function
    ...
    If Not IsEmpty(cellB.value) Then
        ...
        If Left$(valB, 2) = "50" Or Left$(valB, 2) = "45" Or Left$(valB, 2) = "42" Then
            ...
            If dictTransport.Exists(key) Then
                ...
                arr(0) = arr(0) + motoare
                arr(1) = arr(1) + km
                ' Reatribuim (denumirea ram�ne din prima apari?ie ? nu o schimbam)
...
Next i
    ...
    'MsgBox "Statistica a fost generata cu succes �n foaia 'Statistica'.", vbInformation
...
Finalize:
    ...
    Application.EnableEvents = prevEvents
    If Err.Number <> 0 Then
        MsgBox "Eroare �n GenereazaStatistica: " & Err.Description, vbExclamation
    End If
End Sub
...
Public Sub ProtejeazaFoiaSTART()
    ...
    ThisWorkbook.Protect Structure:=True, Windows:=False
    RestoreStructureIfNeeded wasProtWb ' Asigura ca ram�ne protejat
